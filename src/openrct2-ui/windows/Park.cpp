...
#pragma endregion
    ...
    // The scenario's opening text: the park name, the scenario description, and the
    // objective, mirroring what the objective page draws on screen.
    static std::string GetParkObjectiveText()
    {
        auto& gameState = getGameState();

        std::string text = gameState.park.name;
        if (!gameState.scenarioOptions.details.empty())
        {
            text += ". ";
            text += gameState.scenarioOptions.details;
        }

        auto ft = Formatter();
        formatObjective(ft, gameState.scenarioOptions.objective);
        text += ". Objective: ";
        text += OpenRCT2::FormatStringIDLegacy(
            kObjectiveNames[EnumValue(gameState.scenarioOptions.objective.Type)], ft.Data());
    ...
    public:
        ...
        std::string getAccessibilityPageSummary()
        {
            ...
            {
                case WINDOW_PARK_PAGE_ENTRANCE:
                    return "Overview: " + park.name + (Park::IsOpen(park) ? ", open" : ", closed");
                case WINDOW_PARK_PAGE_RATING:
                    return OpenRCT2::FormatStringID(STR_PARK_RATING_LABEL, static_cast<uint16_t>(park.rating));
                ...
                {
                    uint32_t parkSize = park.size * 10;
                    StringId sizeStr = STR_PARK_SIZE_METRIC_LABEL;
                    if (Config::Get().general.measurementFormat == MeasurementFormat::imperial)
                    {
                        sizeStr = STR_PARK_SIZE_IMPERIAL_LABEL;
                        parkSize = SquaredMetresToSquaredFeet(parkSize);
                    }
                    std::string text = OpenRCT2::FormatStringID(sizeStr, parkSize);
                    if (_numberOfRides != -1)
                        text += ", " + OpenRCT2::FormatStringID(STR_NUMBER_OF_RIDES_LABEL, static_cast<uint32_t>(_numberOfRides));
                    if (_numberOfStaff != -1)
                        text += ", " + OpenRCT2::FormatStringID(STR_STAFF_LABEL, static_cast<uint32_t>(_numberOfStaff));
                    text += ", " + OpenRCT2::FormatStringID(STR_GUESTS_IN_PARK_LABEL, static_cast<uint32_t>(park.numGuestsInPark));
                    text += ", " + OpenRCT2::FormatStringID(STR_TOTAL_ADMISSIONS, static_cast<uint32_t>(park.totalAdmissions));
                    return text;
                }
                case WINDOW_PARK_PAGE_OBJECTIVE:
                    return GetParkObjectiveText();
                case WINDOW_PARK_PAGE_AWARDS:
                {
                    auto& awards = park.currentAwards;
                    if (awards.empty())
                        return "No recent awards";
                    std::string text;
                    for (const auto& award : awards)
                    {
                        if (!text.empty())
                            text += ", ";
                        text += OpenRCT2::FormatStringID(AwardGetText(award.type));
                    }
                    return text;
                }
            }
...
#pragma endregion
    ...
    WindowBase* ParkObjectiveOpen()
    {
        ...
        if (wnd != nullptr)
        {
            ...
            // Read the opening text aloud and let the player dismiss it with Enter.
            Accessibility::ScreenReaderSpeak(GetParkObjectiveText());
        }
        return wnd;
    }
