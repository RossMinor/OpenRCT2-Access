...
#pragma region Accessibility
        // Screen-reader navigation (graph recipe, modelled on the Options window). The dropdown pair
        // tracks an open combo box: while set (and the dropdown window really exists), BuildAccessGraph
        // declares the dropdown's items instead of the page's controls.
        ...
        // Stable per-page identity for a control node. Widget indices overlap across pages (each
        // page reuses WIDX_TAB_CONTENT), so the page must be part of the key - without it a tab
        // switch would silently "persist" focus onto an unrelated control of the new page.
        std::string accessControlKey(WidgetIndex w) const
        {
            return "cheat:" + std::to_string(page) + ":" + std::to_string(w);
        }
        ...
        {
            Accessibility::Graph::GraphStateForClass(WindowClass::cheats).nextSuggestedMove
    ...
    public:
        ...
        // dropdown's items) fresh from live widget state. Immediate mode makes the dropdown a plain
        // modal sub-list: no per-window sub-mode dispatch.
        void BuildAccessGraph(Accessibility::Graph::GraphBuilder& b)
        {
            ...
            // Label only, no role word - consistent with the other tabbed windows, which announce
            // just the page name on a tab switch.
        ...
        // Tab/Shift+Tab: cycle the cheat pages via the window's own page switch; the graph
        ...
        // Escape: close an open dropdown sub-list first (staying in the window, focus returning to
        // the combo box); with none open, let the navigator close the window.
        bool AccessEscape()
        {
            if (!_accessDropdownOpen)
                return false;
...
#pragma endregion
    ...
    // Register the cheats window with the graph accessibility navigator (called once at startup via
    ...
    void RegisterCheatsGraphScreen()
    {
        using namespace Accessibility::Graph;
        GraphScreen screen;
        screen.windowClass = WindowClass::cheats;
        screen.screenName = []() { return std::string("Cheats"); };
        screen.build = [](GraphBuilder& b, WindowBase& w) { static_cast<CheatsWindow&>(w).BuildAccessGraph(b); };
        screen.onTabKey = [](WindowBase& w, int32_t dir) {
            static_cast<CheatsWindow&>(w).AccessChangePage(dir);
            return true;
        };
        screen.onEscape = [](WindowBase& w) { return static_cast<CheatsWindow&>(w).AccessEscape(); };
