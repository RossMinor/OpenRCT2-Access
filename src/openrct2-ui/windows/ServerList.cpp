...
namespace OpenRCT2::Ui::Windows
{
    ...
    #pragma region Window Override Events
        ...
        // Keyboard navigation walks the servers, then the three action buttons.
        int32_t getServerCount()
        {
            return static_cast<int32_t>(_serverList.GetCount());
        }

        int32_t getAccessTotal()
        {
            return getServerCount() + 3; // Fetch servers, Add server, Start server
        }

        std::string getServerText(int32_t i)
        {
            const auto& s = _serverList.GetServer(i);
            std::string text = s.Name;
            if (s.MaxPlayers > 0)
                text += ", " + std::to_string(s.Players) + " of " + std::to_string(s.MaxPlayers) + " players";
