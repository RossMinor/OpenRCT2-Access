...
{
    // Tracks the last news item spoken to the screen reader so each is announced once.
    static std::string _lastAnnouncedNewsText;
    ...
    public:
        ...
        void onUpdate() override
        {
            ...
            announceNewsToScreenReader();
        }

        // Speaks the current news item once when it first appears.
        //
        // The onCursor and onPeriodicUpdate overrides that used to sit alongside this went with
        // 0.5.5: upstream split the bottom toolbar into separate panel windows and removed
        // InvalidateDirtyWidgets altogether, so carrying them forward would reference symbols that
        // no longer exist. Only the news announcement is the mod's own.
        static void announceNewsToScreenReader()
        {
            if (News::IsQueueEmpty())
                return;

            const auto* item = News::GetItem(0);
            if (item == nullptr || item->isEmpty() || item->text.empty())
                return;

            if (item->text != _lastAnnouncedNewsText)
            {
                _lastAnnouncedNewsText = item->text;
                Accessibility::ScreenReaderSpeak(item->text);
                Accessibility::LogAnnouncement(item->text);
            }
        }
    };
