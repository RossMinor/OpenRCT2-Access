...
#pragma endregion
    ...
    private:
        ...
        LoadSaveType type;
        // The keypress that opens this window also delivers its key-up here, which would
        // immediately "activate". Swallow that first stray Enter.
        bool _ignoreOpeningEnter = false;
        // Keyboard-navigation cursor, kept separate from selectedListItem (which the mouse
        // hover also writes) so that scrolling under a stationary cursor can't hijack it.
        ...
        void onOpen() override
        {
            ...
            if (isSave)
            {
                Accessibility::ScreenReaderSpeak(
                    "Save game. File name: " + std::string(_currentFilename)
                    + ". Type a name and press Enter to save, or use the arrow keys to choose an existing file.");
            }
        }

        void onClose() override
        {
            // The save dialog leaves a widget textbox session active. If it isn't cancelled,
            // IsUsingWidgetTextBox() stays true after this window is gone and the InputManager
            // swallows every key before it reaches the accessible navigation handlers.
        ...
        void onTextInput(WidgetIndex widgetIndex, std::string_view text) override
        {
            ...
            switch (widgetIndex)
            {
                ...
                case WIDX_FILENAME_TEXTBOX:
                {
                    ...
                    const std::string oldText{ _currentFilename };
                    if (newText == oldText)
                        return;

                    // Echo the change so the user can hear what they typed.
                    if (newText.size() > oldText.size() && newText.compare(0, oldText.size(), oldText) == 0)
                        Accessibility::ScreenReaderSpeak(newText.substr(oldText.size()));
                    else if (newText.size() < oldText.size() && oldText.compare(0, newText.size(), newText) == 0)
                        Accessibility::ScreenReaderSpeak("delete");
                    else
                        Accessibility::ScreenReaderSpeak(newText.empty() ? "blank" : newText);

                    String::safeUtf8Copy(_currentFilename, newText.c_str(), sizeof(_currentFilename));
                }
            }
        }
...
#pragma region Accessibility
        ...
        // Leaf name of the directory currently being browsed, for spoken orientation.
        std::string getDirLeafName() const
        {
            std::string dir = _directory;
            while (!dir.empty() && (dir.back() == '/' || dir.back() == '\\'))
                dir.pop_back();
            const auto pos = dir.find_last_of("/\\");
            std::string leaf = (pos == std::string::npos) ? dir : dir.substr(pos + 1);
            return leaf.empty() ? std::string("Root") : leaf;
        }

        void announceSelectedItem()
        {
            if (_accessIndex < 0 || _accessIndex >= numListItems)
                return;
            const auto& item = _listItems[_accessIndex];
            ...
            else if (!item.dateFormatted.empty())
            {
                text += ", " + item.dateFormatted + " " + item.timeFormatted;
            }
            if (item.loaded)
                text += ", current";
            Accessibility::ScreenReaderSpeakItem(text, _accessIndex, numListItems);
        }
        ...
        void ensureSelectedVisible()
        {
            ...
            const int32_t itemTop = _accessIndex * kScrollableRowHeight;
            const int32_t itemBottom = itemTop + kScrollableRowHeight;
            const int32_t viewHeight = widgets[WIDX_SCROLL].height() - 1;
            if (itemTop < scroll.contentOffsetY)
                scroll.contentOffsetY = itemTop;
            else if (itemBottom > scroll.contentOffsetY + viewHeight)
                scroll.contentOffsetY = itemBottom - viewHeight;
            widgetScrollUpdateThumbs(*this, WIDX_SCROLL);
        }

        void announceFolderEntered()
        {
            _accessIndex = -1;
            selectedListItem = -1;
            Accessibility::ScreenReaderSpeak(
                getDirLeafName() + ", " + std::to_string(numListItems) + (numListItems == 1 ? " item" : " items"));
        }
        ...
        void activateSelectedItem()
        {
            ...
            // Copy what we need: opening a folder repopulates (and clears) the list.
            const auto item = _listItems[_accessIndex];

            if (item.type == FileType::directory)
            {
                char directory[MAX_PATH];
                String::safeUtf8Copy(directory, item.path.c_str(), sizeof(directory));
                numListItems = 0;
                _accessIndex = -1;
                selectedListItem = -1;
                PopulateList(directory, _extensionPattern);
            ...
            {
                Accessibility::ScreenReaderSpeak("Loading " + item.name);
                Select(item.path.c_str(), action, type, _trackDesign);
            }
        }

        // Returns true if this key should be swallowed because it is the stray key-up from
        // the keypress that opened the window. Only the opening Enter is suppressed; any other
        // key clears the guard so a later Enter works normally.
        bool consumeOpeningEnter(uint32_t keycode)
        {
            if (keycode == SDLK_RETURN || keycode == SDLK_KP_ENTER)
            {
                const bool ignore = _ignoreOpeningEnter;
                _ignoreOpeningEnter = false;
                return ignore;
            }
            _ignoreOpeningEnter = false;
            return false;
        ...
        bool isSelectedFolder() const
        {
            return _accessIndex >= 0 && _accessIndex < numListItems
                && _listItems[_accessIndex].type == FileType::directory;
        }
        ...
        void doSave()
        {
            ...
            // Existing file: open the overwrite-prompt window (announced for keyboard users).
            if (File::Exists(path))
            {
                WindowOverwritePromptOpen(_currentFilename, path, action, type, _trackDesign);
                return;
            ...
            const std::string name = _currentFilename;
            auto* windowMgr = GetWindowManager();
            Select(path.c_str(), action, type, _trackDesign);
            // A successful save closes this window; if it's gone, confirm it saved. On failure
            // the window stays open and an error has already been announced.
            if (windowMgr != nullptr && windowMgr->FindByClass(WindowClass::loadsave) == nullptr)
        ...
        bool onAccessibilityAction(AccessibilityAction action_) override
        {
            ...
            switch (action_)
            {
                ...
                case AccessibilityAction::moveDown:
                {
                    ...
                    if (isSave && _listItems[_accessIndex].type == FileType::file)
                    {
                        // Picking a file fills the filename so Enter will overwrite it.
                        String::set(_currentFilename, std::size(_currentFilename), _listItems[_accessIndex].name.c_str());
                        WindowStartTextbox(*this, WIDX_FILENAME_TEXTBOX, _currentFilename, sizeof(_currentFilename));
                        invalidateWidget(WIDX_FILENAME_TEXTBOX);
                    }
                    else if (!isSave)
                ...
                case AccessibilityAction::moveRight:
                    // Right enters a folder (safe in both modes); does nothing on a file.
                    if (isSelectedFolder())
                ...
                case AccessibilityAction::activate:
                    if (isSelectedFolder())
                        activateSelectedItem(); // enter the folder
                    else if (isSave)
                        doSave();
                    else
                        activateSelectedItem(); // load the file
                    return true;
...
#pragma endregion
    ...
    void WindowLoadSaveInputKey(WindowBase* w, uint32_t keycode)
    {
        ...
        // Swallow the stray key-up from the Enter that opened this window, so it doesn't
        // immediately activate (e.g. trigger an overwrite prompt the moment Save opens).
        if (loadSaveWindow->consumeOpeningEnter(keycode))
            return;

        // The InputManager routes all keys here while the loadsave window is open (which also
        // keeps in-game map-cursor keys from firing while typing a filename), so this is where
        // accessible list navigation is dispatched.
        switch (keycode)
        {
            case SDLK_UP:
                loadSaveWindow->onAccessibilityAction(AccessibilityAction::moveUp);
                break;
            case SDLK_DOWN:
                loadSaveWindow->onAccessibilityAction(AccessibilityAction::moveDown);
                break;
            case SDLK_LEFT:
                loadSaveWindow->onAccessibilityAction(AccessibilityAction::moveLeft);
                break;
            case SDLK_RIGHT:
                loadSaveWindow->onAccessibilityAction(AccessibilityAction::moveRight);
                break;
            case SDLK_RETURN:
            case SDLK_KP_ENTER:
                loadSaveWindow->onAccessibilityAction(AccessibilityAction::activate);
                break;
            case SDLK_ESCAPE:
                loadSaveWindow->onAccessibilityAction(AccessibilityAction::cancel);
                break;
        }
    }
} // namespace OpenRCT2::Ui::Windows
