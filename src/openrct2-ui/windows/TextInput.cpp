...
namespace OpenRCT2::Ui::Windows
{
    ...
    private:
        ...
        // Accessibility: announce the prompt once everything is set up, then echo edits.
        bool _accessAnnounced = false;
        u8string _lastSpokenBuffer;
        // Swallow the stray key-up from the Enter that opened this dialog (would otherwise
        // instantly confirm empty text).
        bool _ignoreOpeningEnter = true;

    public:
        // Returns true if this key should be swallowed as the opening Enter. Any other key
        // clears the guard so a later Enter confirms normally.
        ...
        void onUpdate() override
        {
            ...
            // Announce the prompt on the first update, by which point the title, description
            // and initial text have all been set by the open helper.
            if (!_accessAnnounced)
            {
                _accessAnnounced = true;
                announceAccessibilityPrompt();
            }
            else if (_buffer != _lastSpokenBuffer)
            {
                // Echo edits so the user can hear what they type.
                const std::string oldT{ _lastSpokenBuffer.c_str() };
                const std::string newT{ _buffer.c_str() };
                if (newT.size() > oldT.size() && newT.compare(0, oldT.size(), oldT) == 0)
                    Accessibility::ScreenReaderSpeak(newT.substr(oldT.size()));
                else if (newT.size() < oldT.size() && oldT.compare(0, newT.size(), newT) == 0)
                    Accessibility::ScreenReaderSpeak("delete");
                else
                    Accessibility::ScreenReaderSpeak(newT.empty() ? "blank" : newT);
                _lastSpokenBuffer = _buffer;
            }
        ...
        {
            std::string text = (_titleStringId == kStringIdNone) ? _title : OpenRCT2::FormatStringID(_titleStringId);
            std::string desc = (_descriptionStringId == kStringIdNone)
                ? _description
                : OpenRCT2::FormatStringIDLegacy(_descriptionStringId, _descriptionArgs.Data());
            if (!desc.empty())
                text += ". " + desc;
            if (!_buffer.empty())
                text += ". Current text: " + std::string(_buffer.c_str());
            text += ". Type, then press Enter to confirm or Escape to cancel.";
    ...
    void WindowTextInputKey(WindowBase* w, uint32_t keycode)
    {
        ...
        // Swallow the stray key-up from the keypress that opened this dialog.
        if (w->classification == WindowClass::textinput
            && static_cast<TextInputWindow*>(w)->consumeOpeningEnter(keycode))
            return;
        ...
        // Escape cancels the prompt and returns to the parent (move up one level).
        else if (keycode == SDLK_ESCAPE)
        {
            if (w->classification == WindowClass::textinput)
            {
                static_cast<TextInputWindow*>(w)->onMouseUp(WIDX_CANCEL);
            }
        }
