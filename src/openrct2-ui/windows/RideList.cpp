    ...
    public:
        ...
        void announceAccessibilityRide(int32_t index)
        {
            ...
            if (ride != nullptr)
            {
                ...
                const auto status = OpenRCT2::FormatStringIDLegacy(STR_STRINGID, ft.Data());
                if (!status.empty())
                    text += ", " + status;

                // Capacity percentage = current riders / total seats across all trains.
                ...
                if (totalSeats > 0)
                {
                    const int32_t pct = (ride->numRiders * 100) / totalSeats;
                    text += ", " + std::to_string(pct) + "% capacity";
                }
            }
