...
#pragma endregion
    ...
    private:
        ...
        int32_t _fileMenuReopenIndex = -1;
    ...
    public:
        ...
        // canonical left-to-right order, skipping separators and hidden buttons.
        std::vector<WidgetIndex> getAccessibleToolbarItems() const
        {
            std::vector<WidgetIndex> items;
            for (int wi : kWidgetOrderCombined)
            {
                if (wi == WIDX_SEPARATOR)
                    continue;
                if (widgets[wi].type == WidgetType::empty)
                {
                    // The Recent Messages button is hidden on the toolbar by default. Mouse
                    // users toggle it in Options, but keyboard users have no other way to
                    // reach it, so keep it navigable here (except in the editor).
                    if (wi == WIDX_NEWS && !isInEditorMode())
                        items.push_back(static_cast<WidgetIndex>(wi));
                    continue;
                }
                items.push_back(static_cast<WidgetIndex>(wi));
            }
        ...
        static const char* getToolbarItemName(WidgetIndex widgetIndex)
        {
            ...
            {
                case WIDX_PAUSE:
                    return GameIsPaused() ? "Resume game" : "Pause game";
                case WIDX_FASTFORWARD:
                    return "Game speed";
                case WIDX_FILE_MENU:
                    return "File menu";
                ...
                case WIDX_ZOOM_IN:
                    return "Zoom in";
                case WIDX_ROTATE_ANTI_CLOCKWISE:
                    return "Rotate view anticlockwise";
                case WIDX_ROTATE_CLOCKWISE:
                    return "Rotate view clockwise";
                case WIDX_VIEW_MENU:
                    return "View options";
                case WIDX_MAP:
                    return "Map";
        ...
        // The window a toolbar button opens, so we can focus its first item on open.
        static WindowClass getToolbarWindowClass(WidgetIndex widgetIndex)
        {
            switch (widgetIndex)
            {
                case WIDX_CONSTRUCT_RIDE:
                ...
                case WIDX_PARK:
                    return WindowClass::parkInformation;
                case WIDX_STAFF:
                    return WindowClass::staffList;
                case WIDX_GUESTS:
                    return WindowClass::guestList;
                case WIDX_FINANCES:
                    return WindowClass::finances;
                case WIDX_RESEARCH:
                    return WindowClass::research;
                case WIDX_NEWS:
                    return WindowClass::recentNews;
                default:
        ...
        // happened in place). False when it opened a dialog or left the game.
        bool shouldKeepFileMenuOpen() const
        {
            ...
            if (windowMgr == nullptr)
                return false;
            static constexpr WindowClass kDialogs[] = {
                WindowClass::loadsave,    WindowClass::loadsaveOverwritePrompt,
                WindowClass::savePrompt,  WindowClass::scenarioSelect,
                WindowClass::serverList,  WindowClass::about,
                WindowClass::options,     WindowClass::changelog,
                WindowClass::textinput,
            };
            for (auto cls : kDialogs)
            {
                if (windowMgr->FindByClass(cls) != nullptr)
                    return false;
            }
            return true;
        }
                ...
                case WIDX_FILE_MENU:
                    ...
                    switch (selectedIndex)
                    {
                        ...
                        case DDIDX_SAVE_GAME:
                        {
                            ToolCancel();
                            SaveGame();
                            // A quick-save writes straight to the current file with no dialog;
                            // confirm it. If a save-as dialog opened instead (first save), it
                            // announces itself, so stay quiet here.
                            auto* windowMgr = GetWindowManager();
                            if (windowMgr == nullptr || windowMgr->FindByClass(WindowClass::loadsave) == nullptr)
                                Accessibility::ScreenReaderSpeak("Game saved");
                            break;
                        }
