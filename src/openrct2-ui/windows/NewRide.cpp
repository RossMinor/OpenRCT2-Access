...
#pragma region Accessibility
        ...
        int32_t getRideListCount() const
        {
            int32_t count = 0;
            for (const RideSelection* it = _windowNewRideListItems;
                 it->Type != kRideTypeNull || it->EntryIndex != kObjectEntryIndexNull; it++)
            {
                count++;
            }
            return count;
        ...
        std::string getRideName(RideSelection item)
        {
        ...
        std::string getRideListItemText(RideSelection item)
        {
            ...
            if (!(getGameState().park.flags & PARK_FLAGS_NO_MONEY))
            {
                ...
                price = (price >> 16) * rtd.BuildCosts.PriceEstimateMultiplier;
                const StringId stringId = rtd.flags.has(RtdFlag::hasTrack) ? STR_NEW_RIDE_COST_FROM : STR_NEW_RIDE_COST;
            ...
            {
                if (Config::Get().interface.listRideVehiclesSeparately && rideObj != nullptr)
                {
                    text += ". Vehicle: ";
                    text += LanguageGetString(rideObj->GetEntry().naming.Name);
                }
                else
                {
