...
#pragma endregion
    ...
    public:
        ...
        // Spoken description of whatever is currently being researched (Development page).
        std::string getResearchNextItemText()
        {
            const auto& gameState = getGameState();
            if (gameState.researchProgressStage == RESEARCH_STAGE_INITIAL_RESEARCH)
                return OpenRCT2::FormatStringID(STR_RESEARCH_UNKNOWN);
            if (gameState.researchProgressStage == RESEARCH_STAGE_DESIGNING)
                return OpenRCT2::FormatStringID(gameState.researchNextItem->getCategoryName());
            if (gameState.researchNextItem->type == Research::EntryType::ride)
            {
                ...
                if (rtd.flags.has(RtdFlag::listVehiclesSeparately))
                    return OpenRCT2::FormatStringID(gameState.researchNextItem->getName());
                if (gameState.researchNextItem->flags & RESEARCH_ENTRY_FLAG_FIRST_OF_TYPE)
                    return OpenRCT2::FormatStringID(rtd.Naming.Name);
                return OpenRCT2::FormatStringID(gameState.researchNextItem->getName()) + " "
                    + OpenRCT2::FormatStringID(rtd.Naming.Name);
            }
            return OpenRCT2::FormatStringID(gameState.researchNextItem->getName());
        }

        // Spoken description of the most recent completed development (Development page).
        std::string getResearchLastItemText()
        {
            const auto& item = getGameState().researchLastItem;
            if (item->type == Research::EntryType::scenery)
                return OpenRCT2::FormatStringID(item->getName());
            const auto& rtd = GetRideTypeDescriptor(item->baseRideType);
            if (rtd.flags.has(RtdFlag::listVehiclesSeparately))
                return OpenRCT2::FormatStringID(item->getName());
            if (item->flags & RESEARCH_ENTRY_FLAG_FIRST_OF_TYPE)
                return OpenRCT2::FormatStringID(rtd.Naming.Name);
            return OpenRCT2::FormatStringID(item->getName()) + " " + OpenRCT2::FormatStringID(rtd.Naming.Name);
        }
            ...
            switch (page)
            {
                case WINDOW_RESEARCH_PAGE_DEVELOPMENT:
                {
                    if (gameState.researchProgressStage == RESEARCH_STAGE_FINISHED_ALL)
                        return "Overview: All research completed";

                    std::string text = "Overview: Researching " + getResearchNextItemText();
                    text += ", " + OpenRCT2::FormatStringID(ResearchStageNames[gameState.researchProgressStage]);
                    if (gameState.researchProgressStage != RESEARCH_STAGE_INITIAL_RESEARCH
                        && gameState.researchExpectedDay != 255)
                    {
                        text += ", expected " + OpenRCT2::FormatStringID(DateDayNames[gameState.researchExpectedDay]) + " "
                            + OpenRCT2::FormatStringID(DateGameMonthNames[gameState.researchExpectedMonth]);
                    }
                    if (gameState.researchLastItem.has_value())
                        text += ". Last development " + getResearchLastItemText();
                    return text;
                }
